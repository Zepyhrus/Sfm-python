...
import config
import collections
import numpy as np, cv2, os, math, open3d as o3d
...
def extract_features(image_names):
    ...
    for image_name in tqdm(image_names):
        ...
        key_points, descriptor = sift.detectAndCompute(image[..., 0], None)
...
if __name__ == '__main__':
  ...
  for keypoints, image in zip(key_points_for_all, img_names):
    img = cv2.imread(image)
    kpts = [_.pt for _ in keypoints]

    
    circle(img, kpts, rs=2, lw=-1)
    cv2.imshow('_', img)
  ...
  # 这里删除那些为空的点
  structure = structure[~np.isnan(structure).any(axis=1)]
